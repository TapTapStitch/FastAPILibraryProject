...
def create_sample_user(client):
    response = client.post("/api/v1/sessions/sign_up", json=valid_user_data)
...
def test_sign_in_user(client, create_sample_user):
    ...
    response = client.post("/api/v1/sessions/sign_in", json=sign_in_data)
...
def test_sign_in_user_invalid_password(client, create_sample_user):
    ...
    response = client.post("/api/v1/sessions/sign_in", json=invalid_data)
...
def test_show_current_user(authorized_client):
    response = authorized_client.get("/api/v1/sessions/current_user")
    assert response.status_code == status.HTTP_200_OK
...
def test_delete_current_user(authorized_client):
    response = authorized_client.delete("/api/v1/sessions/current_user")
